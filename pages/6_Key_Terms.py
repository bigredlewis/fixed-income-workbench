...
Searchable and organized with collapsible sections.
"""
...
<p style="color: #64748B; font-size: 1.05rem; margin-top: -0.5em; margin-bottom: 1em;">
    Quick reference guide for fundamental fixed income analysis.
</p>
...
search = st.text_input(
    "Search terms",
    placeholder="Type to filter (e.g., duration, leverage, spread...)",
    key="terms_search",
)
...
# --- Term definitions organized by section ---
SECTIONS = {
    "Bond Basics": {
    ...
    "Treasury & Interest Rate Concepts": {
...
for section_name, terms in SECTIONS.items():
    ...
    if search_lower:
        filtered_terms = {
            ...
            if search_lower in term.lower() or search_lower in defn.lower()
        }
        if not filtered_terms:
            continue
        match_count += len(filtered_terms)
    else:
        filtered_terms = terms

    with st.expander(f"{section_name} ({len(filtered_terms)} terms)", expanded=bool(search_lower)):
        for term, definition in filtered_terms.items():
            if search_lower:
                ...
                st.markdown(f"**{term}**")
            st.markdown(f"<span style='color: #4A6A8A; line-height: 1.6;'>{definition}</span>", unsafe_allow_html=True)
            st.markdown("")  # spacing

if search_lower and match_count == 0:
    st.warning(f"No terms found matching \"{search}\".")
elif search_lower:
    st.caption(f"Found {match_count} matching term(s).")
